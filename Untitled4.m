...
filee='C:\Users\Виктор\Desktop\диссертация\2018_1_курс_2_сем\p_09_20_30.dat';

 con_prob = condi_prob(filee);%функция для записи условных вер
 

 % conditional_prob_po_09995=[0.998187680441700 0.998334949205412;0.00181231955829979 0.900523560209424;0 0.00166505079458753;0 0.0995000000000000];
% con_prob_po_0999=[0.997649586284645 0.998297238333422;0.00235041371535533 0.891129032258065;0 0.00170276166657798;0 0.108870967741935];
% con_prob_po_099=[0.994891331637721 0.995837066497673;0.00510866836227882 0.869888475836431;0 0.00416293350232731;0 0.130111524163569];
...
%    nerr=0;%начальное значение количества ошибок
  ...
  % Pe_theor = zeros(size(SNRdBinit : SNRdBincr : SNRdBfin));%массив для теор.вер.ошибки на бит
   ...
   %% подсчет меток ребер и соотвествие меток с состояниями ребер
     ...
     sost_metk  = sostoyanie_metok_reber( k,n,g );%состояния и переходы 
     sost_metk_bigin  = sostoyanie_metok_reber_begin( k,n,g );%%%%%%% самое начало
                                 %% МОДЕЛИРОВАНИЕ
    ...
    param_treils = path_and_metric_struct( k,L );
    
%     struc2 = rmfield(param_treils, 'path_begin  ') - удаление поля
 ...
 %% источник сообщения
       ...
       if r1>r0
                      ...
                      exit_chanel(1,i)=0;
                            ...
                            %изменит это            
%        [ metric,path,b1 ] = vit_algoritm( k,L,sost_metk,exit_chanel,metric,path,takt,r,metric_new,path_new,con_prob(:,:,count) );
        
            [  param_treils,b1 ] = vit_algoritm_magistature( k,L,sost_metk,sost_metk_bigin,exit_chanel,takt,con_prob(:,:,count), param_treils);
