function [ param_treils] = begin_vit(k,sost_metk_bigin,exit_chanel,conditional_prob,param_treils,y)
    ...
    p_e2=conditional_prob(bi2de(rebro1(1,1:2))+1,2);
    m1=log2(p_e1)+log2(p_e2);%метрика ребра 1
    
    p_e1=conditional_prob(bi2de(rebro2(1,1))+1,1);
    p_e2=conditional_prob(bi2de(rebro2(1,1:2))+1,2);
    ...
    %       [ pred_sost_1,bit_pred_1 ] = pred_sostoyanie( k,pred_sost_1 );
        ...
        if c==metrica_nakop_1
            ...
            if c~=-inf
            param_treils.last_bit_rebra_begin(i+1,1)=rebro1(1,end);
            end
            ...
            param_treils.path_new_begin(i+1,:)=param_treils.path_begin(pred_sost_2+1,:);
              ...
              if c~=-inf
           param_treils.last_bit_rebra_begin(i+1,1)=rebro2(1,end);
           end
        end
